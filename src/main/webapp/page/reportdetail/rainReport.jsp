		...
		<table id="detailTable" cellspacing='0' cellpadding='3' class='list-table'>
		 <thead>
			<tr>
				...
				<th width="15%">${viewName1}</th>
				<th width="15%">${viewName3}</th>
				<th width="15%">${viewName2}</th>
			</tr>
		</thead>
		<tbody>
		</tbody>
		...
		if(list==null || list.length==0){
			...
			for (var i in list) {
				if(!isNaN(parseInt(i))){
					...
					newcontent += "<td>"+ (list[i].${factor1}==undefined?'--':list[i].${factor1}) + "</td>";
					newcontent += "<td>"+ (list[i].${factor3}==undefined?'--':list[i].${factor3}) + "</td>";
					if(list[i].${factor2}!=undefined){
						if(list[i].${factor2}=='0'){
							newcontent += "<td><img src='${ctx}/content/skin/css/images/state/rain.png' /></td>";
						}else if(list[i].${factor2}=='1') {
